...
โครงเดียวกับ `esx_core` เป๊ะ ๆ มีห้าคอลัมน์เท่านั้น
...
แต่ละแถวถูกขยายเป็นข้อมูลเต็มชุด เพราะตารางแบบ ESX ไม่มีคอลัมน์รองรับฟิลด์ที่ระบบกระเป๋าต้องใช้:
...
กดใช้แล้วจะเกิดอะไรขึ้นจริงหรือไม่ตัดสินที่ `Core.CreateUseableItem` เหมือน `ESX.RegisterUsableItem`
นอกจากนี้ `hexa_inventory` ยังอ่าน `description` กับ `combinable` จากข้อมูลไอเทมด้วยถ้ามี ตอนลงทะเบียน
...
คำกริยาชุดแคตตาล็อกยังถูก export ด้วยชื่อเดิมแบบ `qb`/`rsg` และเก็บไว้ถาวร สคริปต์ที่พอร์ตมาจะได้ใช้
ได้เลยโดยไม่ต้องแก้:
...
การลงทะเบียน callback คือสิ่งที่ทำให้ไอเทมกดใช้แล้วมีอะไรเกิดขึ้น `hexa_inventory` จะไปหา callback
...
`Core.UseItem(source, item)` ส่งต่อไปยัง `exports['hexa_inventory']:UseItem` และจะออกจากฟังก์ชันพร้อม
เตือนถ้า `hexa_inventory` ยังไม่ started ปกติแล้ว inventory ยิงการใช้ไอเทมเองตอนผู้เล่นกด resource
...
นี่คือเมธอดฝั่งกระเป๋าที่แขวนอยู่บนตัวผู้เล่น ทุกตัวส่งงานต่อให้ `hexa_inventory` และถ้า inventory
...
`Player.SetGang(gang, grade)` มีอยู่จริงและคืน `false` เสมอ ที่ยังเก็บไว้เพราะ bridge ของ rsg เรียกหา
...
`Core.GetTotalWeight(items)` ใช้ชั่งน้ำหนักตารางไอเทมดิบ ๆ และคืน `0` เมื่อ `hexa_inventory` ไม่ทำงาน
