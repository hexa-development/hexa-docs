...
::: tip
สคริปต์ `rsg-core` ที่พอร์ตเข้ามาและวิ่งผ่าน `[bridge]/rsg-core` ไม่เจอปัญหานี้ เพราะ bridge ทำคิวจริงที่คีย์ด้วยชื่อ
**และ** id ของผู้เล่น การเรียก `RSGCore.Functions.TriggerClientCallback` ซ้อนกันจึงปลอดภัยในฝั่งนั้น
...
`Core.UseItem(source, item)` ส่งต่อไปที่ `hexa_inventory` และจะเตือนแล้วไม่ทำอะไรถ้า resource นั้นยังไม่ start
