...
| `items` | table | ช่องเก็บของ ตอนรันไทม์ `hexa_inventory` เป็นเจ้าของข้อมูลชุดนี้ |
...
เมธอดเกี่ยวกับของในตัวบนอ็อบเจกต์ผู้เล่นเป็นตัวส่งต่อไปยัง `hexa_inventory` ถ้า resource นั้นไม่ได้สตาร์ตอยู่
...
| เมธอด | คืนค่า | ค่าที่ได้เมื่อ `hexa_inventory` ไม่ได้สตาร์ต |
| --- | --- | --- |
...
ของฝั่งแค็ตตาล็อกอย่างถาวร เพื่อให้สคริปต์ `rsg-core` ที่พอร์ตมาใช้ได้โดยไม่ต้องแก้
...
โหลดอยู่ เมื่อไอเทมนั้นไม่มีใน `Core.Shared.Items` หรือเมื่อ `hexa_inventory` ไม่ได้สตาร์ต ถ้าไม่ส่ง
...
`users` และสั่งให้ `hexa_inventory` เซฟกระเป๋า ถ้าเป็นอ็อบเจกต์ออฟไลน์จะไปที่
...
`Player.SetGang` มีอยู่เพื่อให้ bridge ของ rsg มีอะไรให้เรียกเท่านั้น มันไม่รับพารามิเตอร์ ไม่ทำอะไร
