...
`Core.Functions` เป็นตารางจริงที่มีสมาชิกอยู่จริง ไม่ใช่ proxy เพราะ bridge ของ RSG ยกฟังก์ชันออกไป
...
ถาม `hexa_inventory` ว่าผู้เล่นเครื่องนี้มี `items` ซึ่งจะเป็นชื่อไอเทมเดี่ยวหรือตารางของชื่อก็ได้
อย่างน้อย `amount` ชิ้นหรือไม่ ถ้า `hexa_inventory` ยังไม่ started จะคืน `false` ทันที
